...
const Highlights = ({stats}) => {
  return (
    <div className='backdrop-blur-lg text-slate-200 flex flex-col justify-start items-center text-transform scale-100 hover:scale-110 transition-transform duration-300 ease-in-out hover:border'>
      <h2 className='text-sm '>{stats.title}</h2>
      <div className='mt-2'>
   ...
   { stats.direction ? (
        ...
        
    {
      stats.title == "Humidity" ? (
        ...
        <div className="bg-blue-600 h-2.5 rounded-full dark:bg-blue-500" style= {{width: `${stats.value}%`}}></div>
      </div>
      ):null
     }
    </div>
